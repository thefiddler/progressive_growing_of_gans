...
# Use GPU or die -- never fall back to CPU.
theano_flags['force_device'] = 'True'
# Use cuDNN or die -- never fall back to custom convolution ops.
theano_flags['dnn.enabled'] = 'True'
# Use float32 for everything by default.
theano_flags['floatX'] = 'float32'
# Give a warning when float64 ops are used by accident  (not sure if this actually works).
theano_flags['warn_float64'] = 'warn'
# Give a warning for CPU ops are used by accident (not sure if this actually works).
theano_flags['assert_no_cpu_op'] = 'warn'
# Conserve memory. Performance loss is alleviated by subsequent flags.
theano_flags['allow_gc'] = 'True'
# Preallocate % of GPU memory to reduce cost at runtime (new gpuarray backend).
theano_flags['gpuarray.preallocate'] = '0.90'
# Preallocate % of GPU memory to reduce cost at runtime (old CUDA backend).
theano_flags['lib.cnmem'] = '0.80'
# Gives the best performance.
theano_flags['dnn.conv.algo_fwd'] = 'small'
# Necessary for deterministic training runs.
theano_flags['dnn.conv.algo_bwd_filter'] = 'deterministic'
# Necessary for deterministic training runs.
theano_flags['dnn.conv.algo_bwd_data'] = 'deterministic'
# Prefer speed over precision.
theano_flags['nvcc.fastmath'] = 'True'
...
train = dict(                               # Training parameters:
    ...
    minibatch_overrides={256: 8, 512: 4,  1024: 2},
    ...
    gdrop_coef=0.0,
    # "/root/dev/pggan/results/003-garfield/network-snapshot-000530.pkl"
...
if 1:
    run_desc = 'dataset2'
    dataset = dict(h5_path='/data/gan/dataset2.h5',
                   resolution=512, max_labels=8, mirror_augment=True,
                   label_path="/data/gan/dataset2-labels.npy")

if 0:
    run_desc = 'garfield'
    dataset = dict(h5_path='/data/gan/garfield.h5',
                   resolution=512, max_labels=0, mirror_augment=False)

if 0:
    run_desc = 'custom-test'
    dataset = dict(h5_path='/data/gan/dataset.h5',
                   resolution=1024, max_labels=0, mirror_augment=True)

# Section 6.3: "High-resolution image generation using CelebA-HQ"
if 0:
    run_desc = 'celeb-hq-1024x1024'
    dataset = dict(h5_path='celeb-hq-1024x1024.h5', resolution=1024,
                   max_labels=0, mirror_augment=True, max_images=30000)
...
if 0:
    ...
    dataset = dict(h5_path=h5_path, resolution=256,
                   max_labels=0, mirror_augment=mirror_augment)
    train.update(lod_training_kimg=800, lod_transition_kimg=800,
                 total_kimg=20000, minibatch_overrides={})
...
if 0:
    ...
    h5_paths = {'celeb128': 'celeb-128x128.h5',
                'bedroom128': 'lsun-bedroom-256x256.h5'}
    dataset = dict(h5_path=h5_paths[datasets[dataset_idx]],
                   max_labels=0, resolution=128, mirror_augment=True)
    ...
    if config_idx < 3:  # gulrajani-et-al-2017 -- small-minibatch
        train.update(D_training_repeats=5, adam_beta2=0.9,
                     minibatch_default=64, G_smoothing=0.999**20)
        G.update(latent_size=128, normalize_latents=False,
                 use_leakyrelu=False, use_batchnorm=True, tanh_at_end=1.0)
        D.update(use_layernorm=True)
        loss.update(iwass_epsilon=0.0)

    if config_idx == 0:  # gulrajani-et-al-2017
        train.update(lod_initial_resolution=128,
                     lod_training_kimg=0, lod_transition_kimg=0)
...
if 0:
    ...
    dataset = dict(h5_path='cifar-10-32x32.h5', resolution=32,
                   max_labels=0, mirror_augment=False)
    train.update(lod_training_kimg=400, lod_transition_kimg=400,
                 rampup_kimg=0, minibatch_overrides={})
    G.update(fmap_base=4096)
    D.update(fmap_base=4096)
...
if 0:
    ...
    run_desc = 'mnistrgb32-k%d-wgangp%s-seed%d' % (
        K, config_postfix, random_seed)
    dataset = dict(h5_path='mnist-rgb-32x32.h5', resolution=32,
                   max_labels=0, mirror_augment=False)
    train.update(D_training_repeats=5, minibatch_default=64,
                 minibatch_overrides={})
    train.update(lod_initial_resolution=4, lod_training_kimg=400,
                 lod_transition_kimg=400, total_kimg=10000)
    G = dict(func='G_mnist_mode_recovery',
             fmap_base=64, fmap_decay=1.0, fmap_max=256)
    D = dict(func='D_mnist_mode_recovery', fmap_base=64,
             fmap_decay=1.0, fmap_max=256, X=K)
    loss = dict(type='iwass', iwass_epsilon=0.0)

    if not useProg:
        train.update(lod_initial_resolution=32,
                     lod_training_kimg=0, lod_transition_kimg=0)
